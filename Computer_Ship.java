/*
 *  By Kevin and Jerry
 *  Computer_ship class -- computer place the ship and human guess the ship
 */
...
public class Computer_Ship {
	...
	int[] ships = {2, 3, 4}; // check if ship sank
	...
	public boolean placeShipHorizontal(int size) { // helper method for place ship (place ship horizontally)
		// the following shipRow and shipColumn are the starting position of a ship
	...
	public void checkBoard() { // This method checks user input if matches with board
		...
		if (guess[rowPos][colPos] == 2) {
			...
			if (ships[board[rowPos][colPos] - 1] == 0) { // checkes if a ship is sank
